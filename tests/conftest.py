...
def configure_test_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, *, expected_dim: int = 2):
    ...
    monkeypatch.setattr(settings, "embedding_trust_remote_code", False)
    ...
    monkeypatch.setattr(settings, "embedding_document_instruction", "")
    monkeypatch.setattr(settings, "min_similarity_threshold", 0.0)
    monkeypatch.setattr(settings, "corrective_rag_enabled", False)
    monkeypatch.setattr(settings, "corrective_rag_max_attempts", 1)
    monkeypatch.setattr(settings, "corrective_rag_min_score", 0.0)
    monkeypatch.setattr(settings, "corrective_rag_min_results", 1)
    monkeypatch.setattr(settings, "corrective_rag_rewrite_timeout_seconds", 8)
    monkeypatch.setattr(settings, "corrective_rag_rewrite_max_tokens", 128)
    monkeypatch.setattr(settings, "deployment_profile", "custom")
    monkeypatch.setattr(settings, "runtime_max_rerank_candidates", 0)
    monkeypatch.setattr(settings, "runtime_max_answer_chunks", 0)
    monkeypatch.setattr(settings, "runtime_retrieval_latency_budget_ms", 0)
    monkeypatch.setattr(settings, "runtime_llm_latency_budget_ms", 0)
    monkeypatch.setattr(settings, "runtime_disable_reranker", False)
    monkeypatch.setattr(settings, "runtime_disable_neural_reranker", False)
    monkeypatch.setattr(settings, "runtime_disable_corrective_rag", False)
    monkeypatch.setattr(settings, "runtime_monitoring_enabled", True)
