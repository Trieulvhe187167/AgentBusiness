...
async def _find_recent_orders_tool(payload: FindRecentOrdersInput, context: RequestContext) -> dict:
    resolved_user_id = _resolve_order_user_id(payload.user_id, context)
    return await find_recent_orders(resolved_user_id, limit=payload.limit)
...
def build_get_order_status_tool() -> ToolSpec:
    return ToolSpec(
        ...
        auth_policy=ToolAuthPolicy(require_user_id=True, scope="order"),
        timeout_seconds=15,
        idempotent=True,
...
def build_get_online_member_count_tool() -> ToolSpec:
    return ToolSpec(
        ...
        auth_policy=ToolAuthPolicy(allow_anonymous=True, scope="game"),
        timeout_seconds=15,
        idempotent=True,
